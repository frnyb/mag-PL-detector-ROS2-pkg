...
MagMeasurementsClass::MagMeasurementsClass(
  mag_pl_detector::msg::MagMeasurements measurements_msg, rotation_matrix_t R_drone_to_mags[4],
    ...
    for (int j = 0; j < 12; j++) {
      ...
      if (j == 0) {
        ...
        if (i == 0) {
          ...
          mag_times_(
            i,
            j) = mag_times_(i - 1, j) + measurements_msg.samples[i].time_offset[j] * 0.00000001;
        ...
        mag_times_(
          i,
          j) = mag_times_(i, 0) + measurements_msg.samples[i].time_offset[j] * 0.00000001;
...
mag_pl_detector::msg::MagneticPhasors3D MagMeasurementsClass::GetPhasorsMsg(
